// This program is a Task Manager application that allows users to manage tasks by performing various operations such as creating, editing, deleting, and viewing tasks.
// It utilizes the Task class to represent individual tasks, and the TaskManager class provides functionalities to interact with tasks, including loading and saving tasks from/to a file.
// Users can perform operations (in main.cpp) such as viewing tasks sorted by date, priority, or category, editing task details such as priority and status, and deleting tasks by their IDs.
// The program also supports color-coded output for task details based on priority and deadline proximity.
...
#include "Task.cpp"  // Include the Task class implementation file
